...
public interface V2Api {
    ...
    @ApiOperation(value = "members", nickname = "v2MembersSearchPost", notes = " * **Purpose**: To retrieve a collection of members based upon the search input * **Prerequisite**: none * **Validation**: Minimum search parameters are sent in ssn or personId or medicareBeneficiaryIdentifier  or firstName and  lastName (together) are the minimum search parameters * **Design Pattern**: Synchronous * **Output**: collection of members (often just a single member)  * **Error Notification**: Various http errors and faults.  If no records match the search input, a http 200 will be returned with an empty members collection.   ", response = ResponseMembersElastic.class, tags={ "Members", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Returns a list of member", response = ResponseMembersElastic.class),
